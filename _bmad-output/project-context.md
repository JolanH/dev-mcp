...
- **`error.code` is a fixed, stable taxonomy:** `BranchExists, WorktreePathInUse, BaseRefNotFound, DirtyWorktree, UnmergedBranch, TaskNotFound, ActiveTaskConflict, LockedWorktree, InvalidTaskName, GitTimeout, InstanceConflict, NotAGitRepo, RollbackIncomplete, PortUnavailable, InvalidStatus, Internal`. (`InvalidStatus` added Story 1.6 for the `update_task` four-state guard + transition matrix; `details.reason ∈ {not_in_set, illegal_transition}`.) Codes are contract; messages may change.
