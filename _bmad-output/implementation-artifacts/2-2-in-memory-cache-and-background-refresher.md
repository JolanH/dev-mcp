---
baseline_commit: ee5de9ad752e74a3a7b5baff97b9082e36adb7ea
---
...
- [x] **Task 1 — Add the two tunables to `config.py`** (AC: 1, 4)
  - [x] `CACHE_REFRESH_INTERVAL: float = 2.0` — background tick period in seconds (**operator-confirmed 2026-06-25, Decision C**). Default 2.0s keeps worst-case staleness under the ≤3s freshness SLO (FR-9 / UX-DR6). Comment it as the tick period (distinct from the dashboard *poll* interval, which is a 2.4b UI concern).
  - [x] No other config change. The READ-pool timeouts/sizes (`GIT_READ_TIMEOUT=3.0`, `GIT_READ_POOL_SIZE=2`, `GIT_READ_ACQUIRE_TIMEOUT=2.0`) already exist (config.py:34-39) and are reused as-is — they ARE the per-repo degrade mechanism.
- [x] **Task 2 — Create `src/dev_helper_mcp/cache.py` (NEW, core/SDK-free)** (AC: 1, 3)
  - [x] Module docstring: ephemeral in-memory derive-on-read cache; the **only** writer of the in-memory view (architecture.md:765-781). Imports only stdlib (`asyncio`, `dataclasses`, `logging`, `collections.abc`), `git/runner` (`GitRunner`, `Pool`), `git/porcelain` (`parse_worktree_porcelain`, `WorktreeEntry`), `store` (`Store`), `projection` (`project`, `CacheSnapshot`), `config`, `util.now_iso`, `errors.DevHelperError`. **NO `mcp`/`starlette`** (auto-policed — `cache.py` is already in `tests/test_adapter_seam.py:19`'s `SEAM_MODULES`).
  - [x] `class Cache:` holds the injected `GitRunner` + `Store` (constructed in the lifespan, loop-bound — Cache does NOT construct them, per the "asyncio objects live in the running loop" rule), plus two private fields: `_current: CacheSnapshot` (seeded to an empty snapshot `CacheSnapshot(generated_at=now_iso(), tasks=(), warnings=())`) and `_last_listings: dict[str, list[WorktreeEntry]]` (per-repo last-successful porcelain, `{}` initially).
  - [x] `@property current(self) -> CacheSnapshot:` returns `self._current` **by reference, no lock** (the read path; `/state` (2.3) and read tools read this — a single ref read is atomic; a swap can never produce a torn snapshot because the snapshot is frozen and replaced whole).
  - [x] `async def refresh(self) -> None:` — the core rebuild. **Total — it never raises** (mirror the projection's totality one layer up; a mutating tool calls it after a committed mutation and must not fail because git is flaky). Algorithm:
...
- [x] **Task 3 — Background refresher loop in `cache.py`** (AC: 1)
...
- [x] **Task 4 — Wire the cache into `ToolDeps` and refresh after every mutation** (AC: 2)
  - [x] `tools/handlers.py`: add `cache: Cache` to the `ToolDeps` dataclass.
  - [x] In the three **mutating** handlers (`create_task`, `remove_worktree`, `update_task`): after the core call returns the success `data` and **before** `return {"ok": True, ...}`, call `await deps.cache.refresh()`. Placement: inside the `try`, after the `data = await ...` line. Because `refresh()` is total it cannot turn a successful mutation into a failure; it only updates the shared cache so `/state` (and the next read) reflects this mutation (AC2). **Do NOT** call refresh in the two **read** handlers (`list_worktrees`, `list_tasks`) — they don't mutate state and `list_worktrees` is itself the live-derive path.
  - [x] Keep each handler's returned `data` **exactly as today** — the architecture's "return the just-built snapshot" (step 7) is satisfied for our tools by the existing per-tool `data` dicts; we are NOT switching tool returns to the snapshot. (`/state`, Story 2.3, is the snapshot consumer.)
- [x] **Task 5 — Wire construction + the background task into the lifespan** (AC: 1, 2)
  - [x] `server_factory.py` `create_app.lifespan`: after opening the `Store`, build the `Cache(runner=..., store=store)` using the **same** `GitRunner` instance the `ToolDeps` get (one runner per app — its pools are the shared concurrency limiter). Order: `runner = GitRunner(); store = await Store.open(); cache = Cache(runner=runner, store=store); holder.deps = ToolDeps(runner=runner, locks=RepoLockRegistry(), store=store, cache=cache)`.
  - [x] Do one **initial** `await cache.refresh()` before launching the loop (so `/state` is warm the instant the server is up, not blank-until-first-tick).
  - [x] Launch the refresher: `refresher = asyncio.create_task(run_refresher(cache, interval=CACHE_REFRESH_INTERVAL))`. On shutdown (the `finally`): `refresher.cancel()` then `await` it inside a `contextlib.suppress(asyncio.CancelledError)` (or try/except CancelledError) so teardown is clean and zombie-free — BEFORE `await store.close()` (the loop touches the store; cancel it first).
  - [x] This is the ONLY `mcp`/`starlette`-importing change in the story (server_factory is adapter). Add NO new route — the cache is in-memory; `/state` is Story 2.3. (Note: deferred-work.md flags that `Mount("/", app=mcp_app)` shadows future routes — that is a **2.3** problem, explicitly NOT 2.2's.)
- [x] **Task 6 — Tests: `tests/test_cache.py`** (AC: 1, 2, 3)
  - [x] **Degrade / stale / carry-forward (AC3) with a FAKE runner — no real git** (fastest, deterministic, zero git-safety surface): inject a stub runner exposing `async def run_git(self, repo, args, *, pool)` that returns a canned `GitResult` for healthy repos and raises `GitTimeout` (or returns `returncode=1`) for a chosen "slow" repo. Assert: (a) a partial-degrade refresh swaps a fresh snapshot whose `warnings` contain exactly `repo_unavailable:<repo>` for the failed repo, and the failed repo's worktrees still render (carried-forward last-known, not orphaned) after a prior successful tick; (b) an **all-repos-unavailable** refresh does NOT advance `current` (same object / same `generated_at` as before — last-known kept); (c) `refresh()` never raises on any failure injected. Pair with a fake/`:memory:` Store (or a tiny stub returning a fixed `list_tasks()` shape).
  - [x] **Atomic swap + by-ref read (AC1):** after a successful `refresh()`, `cache.current` is a new `CacheSnapshot` instance (identity changed) with the projected tasks; two reads of `current` return the same object until the next swap.
  - [x] **End-to-end refresh over real git (AC1)** using the `tmp_git_repo` fixture + a real `GitRunner` + a `:memory:`/tmp `Store`: create a task's worktree (via `core.tasks.create` against the tmp repo, OR seed Store rows + a real `git worktree add` in the tmp repo), `await cache.refresh()`, assert the snapshot groups it correctly (delegates the join correctness to 2.1's tests; here just prove the fan-out → project → swap pipeline works end-to-end on real porcelain). **Git-safety HARD RULE applies** — every git op targets `tmp_git_repo`, never the project repo; the autouse `_guard_project_repo_untouched` + `test_git_safety.py` AST scan enforce it.
  - [x] **Critical-section refresh (AC2)** — drive the in-process ASGI client (`asgi_client_factory`) through the lifespan: call `create_task` against a `tmp_git_repo`, then read `holder.deps.cache.current` (or, once 2.3 lands, `/state`) and assert the new task is present — i.e. the mutation refreshed the cache before returning. (If wiring the ASGI client to a tmp repo is heavy, an equivalent lower-level test: construct `ToolDeps` with a real cache, call `handlers.create_task(...)`, assert `deps.cache.current` reflects it.) Remember `ASGITransport` does NOT auto-run the lifespan — wrap in `async with app.router.lifespan_context(app):` (project-context.md#Testing).
- [x] **Task 7 — Perf/chaos test (AC4), `@pytest.mark.slow`** in `tests/test_cache.py` (or `tests/test_cache_perf.py`)
  - [x] Parametrize `num_tasks` × `repos_per_task` up to ~15 distinct repos. Build the listings via a **fake runner** with an injected `asyncio.sleep(slow_delay)` on one chosen repo (the "slow-repo injector") so the test is deterministic and does NOT need 15 real git repos. Exercise concurrent readers of `cache.current` during a refresh; assert no torn read and that p95 `refresh()` wall-time stays within the ≤3s soft SLO for ≤15 repos. **`log`/assert-document the cliff beyond 15 repos** (e.g. a comment + an explicit assertion that the SLO is bounded, not guaranteed past 15) — do NOT silently cap.
  - [x] Mark `@pytest.mark.slow` (the `slow` marker is registered) so the default gate (`pytest -m "not slow"`) skips it; it runs in the full suite. Time via injected delays / `asyncio` timing, never `Date.now()`-style wall clock assertions that flake.
- [x] **Task 8 — Gate green + seam confirmation** (AC: all)
  - [x] `cache.py` adds no `from mcp …`/`from starlette …` — `tests/test_adapter_seam.py` (already scans `cache.py`) stays green.
  - [x] Full gate: `uv run ruff check . && uv run ruff format --check . && uv run pytest -m "not slow"`. No new runtime dependency (stdlib `asyncio` + existing `aiosqlite`/git). No schema change, no migration, no new HTTP route.

### Review Findings

_Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor), 2026-06-25. 1 decision-needed, 5 patch, 1 deferred, 4 dismissed as noise. **All decision + patch findings resolved** — operator chose the generation-token guard for the race; all 6 patches applied, ruff + format + full suite green (**206 passed**). Reviewer independence caveat: implemented and reviewed in the same session — recommend a confirming pass with a different model._

- [x] [Review][Decision→Patch] Concurrent `refresh()` last-writer-wins could clobber a post-mutation swap with stale data (blind+edge). **RESOLVED (operator chose option 1):** monotonic start-generation guard `_refresh_seq` — each `refresh()` stamps a sequence at entry and only swaps if no newer refresh started while it awaited, so the latest-started (freshest-DB) refresh wins with no added latency. AC2 now airtight. `src/dev_helper_mcp/cache.py:refresh`.
- [x] [Review][Patch] Warm-up `refresh()` + refresher `create_task` ran before the `try/finally` → Store/refresher leak on a startup-cancel. **FIXED:** both moved inside the `try`; `finally` guards `refresher is not None` so `store.close()` always runs [src/dev_helper_mcp/server_factory.py]
- [x] [Review][Patch] `_last_listings` never evicted → growth + stale resurrection. **FIXED:** pruned to the current task-repo set each refresh [src/dev_helper_mcp/cache.py:refresh]
- [x] [Review][Patch] Torn-read test was tautological. **FIXED:** now asserts ≥2 distinct snapshot identities observed across swaps [tests/test_cache.py]
- [x] [Review][Patch] Refresher cancellation-cleanliness was unasserted. **FIXED:** now `pytest.raises(CancelledError)` + `assert task.cancelled()` [tests/test_cache.py]
- [x] [Review][Patch] AC4 perf test single-axis parametrization. **FIXED:** `_build_fanout` gained a `tasks_per_repo` axis; perf test now parametrizes `(num_repos, tasks_per_repo)` = `(5,1),(10,2),(15,3)` [tests/test_cache.py]
- [x] [Review][Defer] `repo_path` spelling/normalization (trailing slash, symlink, pre-canonicalization rows) → same physical repo read twice / link reads orphaned [src/dev_helper_mcp/cache.py:refresh] — deferred, pre-existing (rows are abspath-canonicalized at `create_task`/`remove_worktree`; not introduced by this change)
...
- Full gate green: `uv run ruff check .` ✓, `uv run ruff format --check .` ✓ (41 files), `uv run pytest -m "not slow"` → **201 passed, 5 deselected** (was 190 before; +11 new cache tests). Full slow suite `pytest -m "slow"` → **5 passed** (4 new perf/chaos + the pre-existing real-port uvicorn smoke test, confirming the new lifespan wiring did not regress server startup/teardown).
- `tests/test_adapter_seam.py` green: `cache.py` imports no `mcp`/`starlette` (AST-scanned; the file was already listed in `SEAM_MODULES`).
- One iteration fix during dev: the concurrent-readers slow test used an erroneous `nonlocal stop` for a `run()`-local; replaced with a `{"flag": …}` mutable container.

### Completion Notes List

- **Decision A (adapter-level refresh)** implemented exactly as confirmed: the 3 mutating handlers (`create_task`, `remove_worktree`, `update_task`) call `await deps.cache.refresh()` after the core call returns `ok` and before the envelope. **Zero edits to the reviewed-`done` Epic 1 core mutation paths** (`core/tasks`, `core/worktrees`) — they were not touched.
- **Decision B (shape-preserving degrade)** implemented: per-repo failures surface as `repo_unavailable:<repo_path>` warnings merged via `dataclasses.replace` (2.1's frozen `CacheSnapshot` unchanged — no new fields); successful reads are recorded in `_last_listings` and carried forward on a later failed read so a transient-unavailable repo renders last-known (present), not orphaned; total-failure tick does NOT swap (keeps last-known, lets `generated_at` age); partial degrade DOES swap.
- **Decision C**: `CACHE_REFRESH_INTERVAL = 2.0` added to `config.py`; the existing READ-pool tunables reused as-is (the per-repo degrade mechanism).
- `refresh()` and `_read_repo()` are **total** (never raise) — verified by `test_refresh_never_raises_on_any_injected_failure`, `test_refresh_is_total_when_store_unreadable`, and the totality of the mixed raise/rc=1 case. This is the load-bearing property so a post-mutation refresh can never turn a successful mutation into a reported failure.
- **One `GitRunner` per app**: the lifespan constructs a single runner and hands the same instance to both `ToolDeps` and `Cache` (so the READ-pool concurrency cap stays global). Refresher task is cancelled + awaited (CancelledError suppressed) **before** `store.close()` on teardown.
- No new runtime dependency, no schema change/migration, no new HTTP route (the `/state` endpoint is Story 2.3). `-z` deliberately NOT used (git 2.34 compatibility).
- AC4 perf: p95 `refresh()` wall-time within the ≤3s SLO for 5/10/15 repos with a slow-repo `asyncio.sleep` injector and a sem=2 throttle mirroring the real READ pool; the parametrization stops at 15 with an explicit assertion + comment documenting that the latency cliff beyond 15 repos is acknowledged, not guaranteed (no silent cap).

### File List

- `src/dev_helper_mcp/cache.py` — **NEW**: `Cache` (ephemeral in-memory derive-on-read view; `current` by-ref read, total `refresh()`, total `_read_repo()`) + `run_refresher` background loop.
- `src/dev_helper_mcp/config.py` — **MODIFIED**: added `CACHE_REFRESH_INTERVAL = 2.0`.
- `src/dev_helper_mcp/tools/handlers.py` — **MODIFIED**: `ToolDeps.cache: Cache` field; `await deps.cache.refresh()` in the 3 mutating handlers (not the 2 read handlers).
- `src/dev_helper_mcp/server_factory.py` — **MODIFIED**: lifespan builds a shared `GitRunner` + `Cache`, warms it with an initial `refresh()`, launches `run_refresher`, and cancels+awaits it before `store.close()`.
- `tests/test_cache.py` — **NEW**: AC1 (atomic swap/by-ref, empty-not-stale), AC3 (partial degrade + carry-forward, no-last-known orphan, total-fail keep-last-known, store-unreadable totality, mixed-failure totality, distinct-repo dedup), refresher loop, AC1/AC2 real-git pipeline (`tmp_git_repo`) + critical-section mutation refresh, AC4 `@pytest.mark.slow` perf/chaos + concurrent-reader no-torn-read.
- `tests/test_handlers.py` — **MODIFIED**: `_deps` helper now constructs a `Cache` (shared runner) so the existing handler tests build a valid `ToolDeps` and exercise the new refresh hook.
- `_bmad-output/implementation-artifacts/2-2-in-memory-cache-and-background-refresher.md` — story frontmatter `baseline_commit`, task checkboxes, this record, Status.
...
| Date | Change |
| --- | --- |
| 2026-06-25 | Story 2.2 implemented: NEW `cache.py` (`Cache` + `run_refresher`) over 2.1 `project()`; `CACHE_REFRESH_INTERVAL=2.0`; post-mutation `cache.refresh()` in the 3 mutating handlers (adapter-level, Decision A); per-repo READ-pool fan-out with independent degrade (`repo_unavailable:` warnings + carry-forward last-known, don't-swap-on-total-fail, Decision B); lifespan wiring (shared `GitRunner`, warm-start, launch/cancel refresher). Tests: `tests/test_cache.py` (AC1-4) + `_deps` helper update. Gate green (201 passed / 5 slow passed); SDK seam green; no new dep/schema/route. Status → review. |
